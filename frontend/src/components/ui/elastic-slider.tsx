...
export const ElasticSlider: React.FC<ElasticSliderProps> = ({
  ...
  isStepped = true,
  stepSize = 500,
  leftIcon = <Minus className="w-4 h-4 text-blue-500" />,
  rightIcon = <Plus className="w-4 h-4 text-blue-500" />
}) => {
  return (
  ...
  return (
    <>
      <motion.div
        ...
        style={{
          scale,
          opacity: useTransform(scale, [1, 1.05], [0.85, 1])
        }}
      ...
      <p className="value-indicator text-white">₹{Math.round(value).toLocaleString()}</p>
    </>
  );
};
